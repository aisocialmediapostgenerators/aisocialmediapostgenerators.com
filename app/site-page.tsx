...
export function SitePage({ tool }: { tool: Tool }) {
  ...
  return (
    <main>
      ...
      <footer><div className="shell footer-top"><div><a className="brand light-brand" href="/"><span className="brand-mark"><Sparkle small /></span> Social Post AI</a><p>Better words. Brighter ideas.<br />Less time staring at a blank box.</p></div><div><b>Tools</b>{toolList.slice(0, 3).map((item) => <a key={item.slug} href={`/${item.slug}/`}>{item.shortTitle}</a>)}</div><div><b>Company</b><a href="/about/">About</a><a href="/terms/">Terms</a><a href="/refund/">Refund policy</a></div></div><div className="shell footer-bottom"><span>© 2026 Social Post AI</span><span>Made for people with something to say.</span></div></footer>
    </main>
  );
}
